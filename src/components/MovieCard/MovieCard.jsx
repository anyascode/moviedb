import { Card, Tag, Col, ConfigProvider, Image, Rate, Row } from 'antd';
...
function MovieCard({ title, description, posterUrl, date, vote, genres, id }) {
  const context = useContext(TheMovieDBContext);

  async function handleRate(rating) {
    await context.rateMovie(id, rating);
  }
  ...
  const rating = context.ratings[id];
      ...
      <Card className="movie-card">
        <Row>
          ...
          <Col xl={14} sm={16} xs={18} md={15} className="movie-info">
            ...
            <p className="movie-description">{truncateString(description, 20)}</p>
            <Rate count={10} onChange={handleRate} value={rating} className="movie-rate" />
          </Col>
        </Row>
      </Card>
    </ConfigProvider>
